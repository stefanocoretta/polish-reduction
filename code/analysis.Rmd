...
midpoint_wide <- readRDS("./data/derived/midpoint_wide.rds")
formant_midpoint <- readRDS("./data/derived/formant_midpoint.rds")
```
...
The figure below illustrates the PC1-PC2 rotation for all individual speakers.

```{r pc-ind}
PC_ind <- midpoint_rotated %>%
  group_by(speaker, V1) %>%
  summarize(
    PC1 = mean(PC1),
    PC2 = mean(PC2)
...
ind.PCA.plot <- PC_ind %>%
  ...
  ggplot() +
   ...
   # geom_polygon(color = "black", fill = NA, size = 0.25, linetype = "dashed") +
    ...
    ggtitle("Individual PCA results") +
  facet_wrap(~speaker)
...
ggsave(file = "./figures/Figure3.pdf", width = 7, height = 5, device = cairo_pdf)
```
...
Figure 4 from the paper with boxplots and means of vowel duration depending on speech rate and stress.
...
```{r save-rate-stress}
ggsave(dur.stress.rate, file = "./figures/Figure4.pdf", width = 6, height = 3, device = cairo_pdf)
```
...
ggsave(file = "./figures/Figure5.pdf", height = 6.5, width = 7, device = cairo_pdf)
```
...
ggsave(dur, file = "./figures/Figure6.pdf", width = 7, height = 4, device = cairo_pdf)
```
...
z1.dur.ef <- ef1 %>%
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  geom_line() +
  geom_point(data = midpoint_rotated_ef2, aes(duration, z1), alpha = 0.08) +
  #geom_line(aes(duration, -upper, color=stress), linetype=2)+
  #geom_line(aes(duration, -lower, color=stress), linetype=2)+
  #scale_color_grey()+
  facet_wrap( ~ V1) +
...
z2.dur.plot <- ef1 %>%
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  geom_line() +
  geom_point(data = midpoint_rotated_ef2, aes(duration, z2), alpha = 0.08) +
  #geom_line(aes(duration, -upper, color=stress), linetype=2)+
  #geom_line(aes(duration, -lower, color=stress), linetype=2)+
  #scale_color_grey()+
  facet_wrap( ~ V1, ncol = 2) +
...
## Modelling after excluding slow speech rate data
### F1

```{r f1-lmer_noslow}

midpoint_no_slow <- midpoint_rotated %>%
  filter(rate!='slow')
...
midpoint_rotated_ef1 <- midpoint_no_slow %>%
  mutate(V1 = factor(V1, levels = c('i', 'ɨ', 'u', 'e', 'a', 'o')))
...
midpoint_rotated_ef2 <- midpoint_no_slow %>%
  mutate(V1 = factor(V1, levels=c('i','u','e','o','ɨ','a')))
...
midpoint_rotated_z1 <- midpoint_no_slow %>%
  mutate(V1 = factor(V1, levels = c('i', 'ɨ', 'u', 'e', 'a', 'o')))
...
midpoint_rotated_z2 <- midpoint_no_slow %>%
  mutate(V1 = factor(V1, levels=c('i','u','e','o','ɨ','a')))
...
This function takes a wide spline dataset as input, where spline data are in Cartesian coordinates named X_1:Y_42. This function drops fanlines with missing data, and performs a PCA on the rest of the data
...
tongue_contour_pca <- function(df){
  ...
  df$PC3 <- predict(tongue.pca, df)[,3]
  df$PC4 <- predict(tongue.pca, df)[,4]
  ...
  df$PC3_var <- as.numeric(summary(tongue.pca)$importance[2,][3])
  df$PC4_var <- as.numeric(summary(tongue.pca)$importance[2,][4])
  return(df)
}
```
...
rotate_PCs <- function(df){
  f1.lm =lm(f1.z~PC1+PC2 + PC3 + PC4, data=df)
  df$comp1 = predict(f1.lm, df)
  
  f2.lm =lm(f2.z~PC1+PC2 + PC3 + PC4, data=df)
  df$comp2 = predict(f2.lm, df)
  return(df) 
}
```

Run PCA for each speaker in dataset
...
Join with acoustic data and rotate by-speaker.

```{r join}
midpoint_rotated_4PCs <- pca_output %>%
  select(speaker,
         item,
         block,
         PC1,
         ...
         PC4_var) %>%
  ### add formant info
  inner_join(formant_midpoint[, c('speaker', 'item', 'block', 'f1.z', 'f2.z')], by =
               c('speaker', 'item', 'block')) %>%
  group_by(speaker) %>%
  do(rotate_PCs(.)) %>%
  ungroup %>%
  select(speaker:PC4_var, z1 = comp1, z2 = comp2) %>%
  inner_join(formant_midpoint, by = c('speaker', 'item', 'block'))

midpoint_rotated_4PCs %>%
  select(speaker, PC1_var:PC4_var) %>%
  unique() %>%
  mutate(combined_var = PC1_var + PC2_var + PC3_var + PC4_var) %>%
  summarise(mean.comb = mean(combined_var))

midpoint_rotated_4PCs <- midpoint_rotated_4PCs %>%
  mutate(
    rate = fct_recode(
      rate,
      slow = "Slow",
    ...
    stress = fct_recode(stress, unstressed = "pre-stressed"),
    item = factor(item),
    V1 = str_replace(V1, "y", "ɨ"),
    V1 = factor(V1, levels = c("a", "e", "i", "o", "u", "ɨ")),
    frame = item,
...
levels(midpoint_rotated_4PCs$frame) <- c(
  "b_b",
  "b_b",
  "b_w",
  "b_w",
...
z_sum <- midpoint_rotated_4PCs %>%
  group_by(V1, rate, stress) %>%
  summarise(
    mean.comp1 = mean(z1, na.rm = T),
...
z_sum %>%
  ...
  theme(plot.margin = unit(c(0, 0, 0, 0), "lines")) +
  facet_wrap( ~ rate) +
  labs(title = "(B) Predicted vowel space",
       y = "Z1",
       x = "Z2") +
  theme(plot.margin = unit(c(0, 0, 0, 0), "lines"))
```
...
No significant effect of stress emerges.
