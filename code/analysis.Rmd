...
acc.plot<-acc.vowel.plot%>%
  ...
  scale_y_reverse("F1.z", limits=c(2.5, -1.5))+
  scale_fill_grey()+
  theme_minimal()+
  geom_point()+
...
```{r save-acc-art}
ggarrange(acc.plot, z1z2.plot, ncol=1)
...
f1.dur.plot <- ef1 %>%
  ...
  aes(duration, fit)+
  #add data
  geom_point(data=midpoint_rotated, aes(duration, f1.z),alpha=0.5)+
...
f1.stress.plot<-ef2 %>%
  ...
  theme_bw()+
  scale_x_discrete('vowel')+
  scale_y_continuous('F1.z')

f1 <- ggarrange(f1.dur.plot, f1.stress.plot, ncol=2)
...
ef1 <- as.data.frame(effect("V1:duration", f2.lmer.final)) %>%
...
f2.dur.plot <- ef1 %>%
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha=0.3)+
  geom_line()+
  geom_point(data=midpoint_rotated, aes(duration, f2.z),alpha=0.5)+
  #geom_line(aes(duration, -upper, color=stress), linetype=2)+
  #geom_line(aes(duration, -lower, color=stress), linetype=2)+
  #scale_color_grey()+
...
ggsave(f2.dur.plot, file = "./figures/Figure6.pdf", width = 3, height = 4, device = cairo_pdf)
...
z1.dur.ef<-ef1 %>%
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha=0.3)+
  geom_line()+
  geom_point(data=midpoint_rotated, aes(duration, z1),alpha=0.5)+
  #geom_line(aes(duration, -upper, color=stress), linetype=2)+
  #geom_line(aes(duration, -lower, color=stress), linetype=2)+
  #scale_color_grey()+
  facet_wrap(~V1)+
...
ggsave(z1.dur.ef, file = "./figures/Figure7.pdf", width = 7, height = 5, device = cairo_pdf)
```
...
ggsave(z2.dur.plot, file = "./figures/Figure8.pdf", width = 4, height = 5, device = cairo_pdf)
```
...
```{r f1-lmer-tab}
tab_model(f1.lmer.final, digits = 4, digits.p = 5, digits.re = 4)
```

## F2

```{r f2-lmer-tab}
tab_model(f2.lmer.final, digits = 4, digits.p = 5, digits.re = 4)
```

## Z1

```{r z1-lmer-tab}
tab_model(z1.lmer.final, digits = 4, digits.p = 5, digits.re = 4)
```

## Z2

```{r z2-lmer-tab}
tab_model(z2.lmer.final, digits = 4, digits.p = 5, digits.re = 4)
```
