...
midpoint_long <- midpoint_wide %>%
  ...
  mutate(fan_line = as.numeric(fan_line))
```
...
```{r pitch}
pitch <- read_csv("./data/raw/pitch.csv", na = "--undefined--") %>%
  mutate(
    item = str_replace(item, "�", "o")
  )

pitch$start = round(pitch$start, 4)
pitch[pitch$item == 'ma' & pitch$start == 453.3759,]$item <- 'bywa'
pitch[pitch$item == 'ma' & pitch$start == 470.9234,]$item <- 'zywo'
pitch[pitch$item == 'ma' & pitch$start == 473.6989,]$item <- 'kuma'
...
pitch <- pitch %>%
  ...
  group_by(speaker) %>%
  mutate(f0.z = scale(f0)) %>%
  ungroup()
```

```{r save-pitch}
saveRDS(pitch, file = "./data/derived/pitch.rds")
```
...
midpoint_rotated <- pca_output %>%
  ...
  left_join(y = pitch %>% select(speaker, item, block, f0, f0.z), by = c("speaker", "item", "block"))
```
